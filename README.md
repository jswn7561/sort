...
## 小组分工
  ...
  * 基础排序算法实现——王乙庚
    ...
    分析并实现5种基础排序算法，同时每种排序算法提供两个版本的实现。

  * 大数字排序算法实现——孔德斌

    实现5种基础排序算法的大数字版本，大数字使用字符串实现。

  * 分布式排序算法实现——陈亚光

    将王乙庚同学实现的排序算法改为可以对数组的一部分进行排序，实现两个版本的分布式排序。
...
对数值范围[-10^100, 10^100]的大数进行排序，数据需要以字符串的形式存储。

对于基于比较的排序算法，需要实现比较函数；
对于基数排序，直接按照字符串中相应位排序即可。
...
本方法提供同步分布式排序，调用本方法后会将待排序数据分成10份，每份单独在一个线程上执行排序，10个线程分别采用BaseSort中的10种排序方法，所有线程排序完成后再将排序结果归并到一起，排序过程中会阻塞主线程，直到排序完成。
